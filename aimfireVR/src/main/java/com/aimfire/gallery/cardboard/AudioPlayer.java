/*
 * code adapted from https://github.com/taehwandev/MediaCodecExample
 */
...
public class AudioPlayer 
{
	...
	{
		//int volume = AudioConfigure.getMaxAudioVolume();
        //AudioConfigure.setAudioVolume(volume/4);
		...
		for (int i = 0; i < mExtractor.getTrackCount(); i++) 
		{
			...
			if (mime.startsWith("audio/")) 
			{
				...
				if(BuildConfig.DEBUG) Log.d(TAG, "format : " + format);
				ByteBuffer csd = format.getByteBuffer("csd-0");
				
				for (int k = 0; k < csd.capacity(); ++k) 
				{
					if(BuildConfig.DEBUG) Log.d(TAG, "csd : " + csd.array()[k]);
				}
		...
		if (mDecoder == null) 
		{
			if(BuildConfig.DEBUG) Log.e("DecodeActivity", "Can't find video info!");
			return;
		}
	...
	private MediaFormat makeAACCodecSpecificData(int audioProfile, int sampleRate, int channelConfig) 
	{
	    ...
	    for (int i = 0; i < samplingFreq.length; ++i) 
	    {
	    	    ...
	    	    {
	    		    if(BuildConfig.DEBUG) Log.d(TAG, "kSamplingFreq " + samplingFreq[i] + " i : " + i);
	    		    sampleIndex = i;
	    	    }
	    }
		...
		while (!eosReceived) 
		{
			...
			if (inIndex >= 0) 
			{
				...
				if (sampleSize < 0) 
				{
					...
					// dequeueOutputBuffer
					if(BuildConfig.DEBUG) Log.d("DecodeActivity", "InputBuffer BUFFER_FLAG_END_OF_STREAM");
				...
				case MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED:
					if(BuildConfig.DEBUG) Log.d("DecodeActivity", "INFO_OUTPUT_BUFFERS_CHANGED");
					outputBuffers = mDecoder.getOutputBuffers();
					break;
					
				case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
					MediaFormat format = mDecoder.getOutputFormat();
					if(BuildConfig.DEBUG) Log.d("DecodeActivity", "New format " + format);
					audioTrack.setPlaybackRate(format.getInteger(MediaFormat.KEY_SAMPLE_RATE));
					
					break;
				case MediaCodec.INFO_TRY_AGAIN_LATER:
					if(BuildConfig.DEBUG) Log.d("DecodeActivity", "dequeueOutputBuffer timed out!");
					break;
					
				default:
					ByteBuffer outBuffer = outputBuffers[outIndex];
					//if(BuildConfig.DEBUG) Log.v("DecodeActivity", "We can't use this buffer but render it due to the API limit, " + outBuffer);
				...
				{
					if(BuildConfig.DEBUG) Log.d("DecodeActivity", "OutputBuffer BUFFER_FLAG_END_OF_STREAM");
					break;
				}
			}
		}
